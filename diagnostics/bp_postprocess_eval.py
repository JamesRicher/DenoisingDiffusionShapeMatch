...
Takes the calibrated BP scalars as given, applies belief_prop.bp_refine once to each
sampled assignment, and reports sparse error/accuracy plus the dense whole-shape MGE and
PCK AUC after densification. No learning, no gradients.

The distinction matters, because a sparse gain does not automatically survive
densification: near-misses are cheap against the quantisation floor.

BP is injected by wrapping model.validate_single, the single point where every reporting
path gets its sparse map. Everything downstream is the model's own validation() verbatim,
so the baseline column reproduces evaluate.py and the two columns differ only by the
refinement.

Variables are placed on Y (rows of P_t): source features and metric from Y, labels and
label metric from X.

Sparse-sampling regimes (--regime):
  bijective   -- sparse Y point j is the GT image of sparse X point j, so sparse
                 avg_error/acc are defined. GT enters point selection, so the dense MGE is
                 optimistic; a dev diagnostic, not a table number.
  independent -- each shape FPS'd on its own geometry, no GT in selection. Only dense
                 MGE/AUC are produced, and a densifier is required.
Both columns of a comparison share one regime, so the base->BP delta is meaningful either
way; only the absolute numbers differ in meaning.
...
Defaults are the calibrated global scalars. Per-dataset optima differ, so pass them
explicitly rather than trusting one global default.
"""
import argparse
import json
import os
