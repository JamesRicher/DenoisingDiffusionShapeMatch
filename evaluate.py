...
from train import _single_collate, _RestoringLoader, seed_everything
...
def results_root_for(opt, args, ckpt):
    """The results/ root this eval writes under, decided purely by the checkpoint so that one
    model directory owns all of its evaluations:

      * a --checkpoint  -> <checkpoint's experiment root>/results
      * else            -> opt['path']['results']  (config's own model == checkpoint)

    -n/--name does not enter here: it names the results/ leaf (the eval_tag), not the root,
    so evaluating one model on several datasets never clobbers. The checkpoint root is two
    levels up from .../models/<file>.pth; a non-standard layout falls back to the config's
    dir with a warning."""
