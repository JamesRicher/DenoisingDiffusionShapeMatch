...
A densifier is a non-learned post-process (kept out of the training loss):
...
class DensifyContext:
    ...
    evecs_x, evecs_y, evals_x, evals_y, mass_x, mass_y: spectral operators (functional map).
    """
    idx_x: torch.Tensor
    idx_y: torch.Tensor
    n_x: int
...
class BaseDensifier(ABC):
    ...
    FEAT_SOURCES = ('diffnet',)

    def __init__(self, opt: Optional[dict] = None):
        self.opt = opt or {}
        # Feature source for the data term: 'diffnet' is dense per-vertex DiffusionNet
        # descriptors from the model's trained extractor (extract_dense). The densifier
        # cannot produce them itself, so the model fills ctx.feat_x/feat_y before densify().
        self.feat_source = self.opt.get('feat_source', 'diffnet')
        ...
        # the model must fulfil this by densely running its own extractor before densify()
        self.wants_model_feats = True
