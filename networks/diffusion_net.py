"""DiffusionNet feature extractor (Sharp et al.), specialised to this codebase:

  * Single-shape and unbatched: the model runs the extractor one shape at a time, so
    every op works on 2-D [V, C] tensors.
  * Operators are pre-cached. The dataset emits evecs / evals / mass / gradX / gradY per
    item under ret_evecs, so forward takes the operators rather than recomputing them.
  * Spectral diffusion only; implicit_dense is not ported.

DiffusionNetExtractor is the registered network. It sets needs_operators = True, so the
model calls extract(shape_dict, idx) with the operators from the shape dict.

Ported from ULRSSM which is turn based on the original implementation.
"""
import math
...
class DiffusionNetExtractor(nn.Module):
    ...
    Consumes the shape dict (for its cached spectral operators) rather than
    (verts, dist): the model branches on needs_operators.
