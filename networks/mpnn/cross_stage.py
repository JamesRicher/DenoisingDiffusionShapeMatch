"""Cross stage: inter-shape feature exchange.
  ...
  u:  (B, n_y, n_x) current log-assignment state (soft-projected, rows = Y),
      the same orientation as P_t.
  cand_*: static feature-kNN candidate indices.

CrossMPNNExchange: hard-cutoff bipartite GNN. Candidate set per
receiver = static feature-kNN and dynamic top-k of the current state (the union
...
def build_cross_stage(dim: int, k_state: int, n_rbf: int,
                      mlp_ratio: float, dropout: float) -> nn.Module:
    """Construct the cross stage."""
