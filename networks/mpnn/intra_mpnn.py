"""Intra-shape stage: an anisotropic geodesic MPNN layer.

One layer = two AdaLN-Zero sublayers:
...
EdgeConv messages (Wang et al. 2019) with a multiplicative per-channel distance
filter (SchNet cfconv) and GraphSAGE-style mean-max aggregation. No softmax: a
hard-cutoff spatial GNN, not a biased transformer.

The distance RBF is static, precomputed once per denoiser call. One instance per
depth is shared across both shapes, preserving pair-swap symmetry. AdaLN-Zero
gates start at 0, so the layer is identity at init.
"""
import torch
import torch.nn as nn

from networks.denoiser_blocks import AdaLNModulation, FeedForward, modulate
