...
class ShapeCache:
    ...
    The heavy per-shape entries (the full geodesic distance matrix and the spectral
    operators) are held in bounded LRU stores, so worker RAM stays flat however many
    distinct shapes an epoch touches. The light entries (verts/faces/corres) stay
    unbounded.

    Args:
        dist_maxsize: max distinct geodesic matrices kept (LRU). None => unbounded. The
            bound is per worker, so the footprint is dist_maxsize * matrix * num_workers;
            a miss only costs the .mat read.
        ops_maxsize: max distinct spectral-operator sets kept (LRU). Left larger, since an
            operator set is far smaller than a distance matrix.
    """
    ...
    def __init__(self, dataset, exclude_self=False):
        ...
        Args:
            ...
            exclude_self (bool): drop the diagonal (i, i) self-pairs. Self-pairs are
                ...
                evaluation metrics; the frozen-feature baselines skip them, so set this
                True for val/test to keep results comparable. Default False.
        """
...
class SparsePairShapeDataset(PairShapeDataset):
    ...
    Wraps a SingleShapeDataset like PairShapeDataset and keeps the full 'first'/'second'
    dicts (needed for eval-time densification). Per shape it adds a 'sparse' sub-dict
    (feat, dist, verts, idx) holding n_sparse points chosen by FPS on X and pushed through
    the shared template to Y, so the sparse GT is bijective by construction. The template
    is whatever indexes both shapes' corr: a shared .vts for FAUST/SCAPE/SMAL/DT4D, or X's
    own vertex set when the GT is a per-pair dense map (SHREC19). Pair-level fields
    ('gt_perm', 'fps_idx') sit at the top level.

    Args:
        dataset: a SingleShapeDataset with ret_corr and ret_dist True.
        n_sparse: number of sparse points per shape.
        phase: "train" randomises the FPS start each item (augmentation); anything else
            uses a fixed index-derived start for comparable val/test numbers.
        exclude_self: forwarded to PairShapeDataset (drop the (i, i) self-pairs).

    independent_fps (attribute, default False): eval-only mode in which each shape is FPS'd
    on its own geometry, so no ground truth enters instance construction and there is no
    bijective sparse target. Emits sparse tokens but no gt_perm/fps_idx, so only dense MGE
    is a valid score. Flipped on solely by evaluate.py, so it cannot leak into training.
    """
    # False: both shapes carry a 'corr' into a shared template (ret_corr). True: no shared
    # template, so _pair_gt attaches a per-pair corr instead (SHREC19).
    ...
    def _sparse_view(shape, idx):
        """Per-shape sparse token dict for the n picked vertices."""
        view = {
            'idx': idx,
