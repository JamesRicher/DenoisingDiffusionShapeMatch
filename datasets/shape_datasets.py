...
class SingleFaustDataset(SingleShapeDataset):
    ...
    def __init__(self,
                 ...
                 ret_evecs=False,
                 num_evecs=200):
        super().__init__(data_root, ret_faces, ret_corr, ret_dist, ret_evecs, num_evecs)
...
class SingleSmalDataset(SingleShapeDataset):
    ...
    def __init__(self,
        ...
        super(SingleSmalDataset, self).__init__(data_root, ret_faces, ret_corr, ret_dist, ret_evecs, num_evecs)
...
class SingleScapeDataset(SingleShapeDataset):
    def __init__(self,
                 ...
                 ret_evecs=False,
                 num_evecs=200):
        super(SingleScapeDataset, self).__init__(data_root, ret_faces, ret_corr, ret_dist, ret_evecs, num_evecs)
        assert phase in ['train', 'test', 'full'], f'Invalid phase {phase}, only "train" or "test" or "full"'
...
class PairFaustDataset(PairShapeDataset):
    def __init__(self,
                 ...
                 num_evecs=200,
                 exclude_self=False):
        dataset = SingleFaustDataset(data_root, phase, ret_faces, ret_corr, ret_dist, ret_evecs, num_evecs)
...
class SparsePairFaustDataset(SparsePairShapeDataset):
    ...
    def __init__(self,
                 ...
                 exclude_self=False,
                 fps_metric="geodesic"):
        dataset = SingleFaustDataset(data_root, phase, ret_faces=True, ret_corr=True, ret_dist=True, ret_evecs=ret_evecs,
                                     num_evecs=num_evecs)
...
class PairSmalDataset(PairShapeDataset):
    def __init__(self,
                 ...
                 num_evecs=200,
                 exclude_self=False):
        dataset = SingleSmalDataset(data_root, phase, category, ret_faces, ret_corr, ret_dist, ret_evecs, num_evecs)
...
class _Shrec19Pairs:
    ...
    SHREC19 ships a fixed set of directed dense pointwise maps (corres/{i}_{j}.map) rather
    than a shared template, so pairs come from the files on disk and corr is attached per
    pair. GT is index-aligned: first['corr'] = arange(V_first) and second['corr'][k] is the
    target vertex of source vertex k.

    That per-pair corr is still a valid template for bijective sparse sampling, with X's own
    vertex set as the template. A shared template would additionally give the reverse
    direction, which SHREC19 lacks since the maps are directed.
...
class PairShrec19Dataset(_Shrec19Pairs, PairShapeDataset):
    ...
    def __init__(self,
        ...
        super().__init__(SingleShrec19Dataset(data_root, ret_faces=ret_faces, ret_corr=False, ret_dist=ret_dist,
                                              ret_evecs=ret_evecs, num_evecs=num_evecs))
...
class SparsePairShrec19Dataset(_Shrec19Pairs, SparsePairShapeDataset):
    ...
    def __init__(self,
        ...
        super().__init__(SingleShrec19Dataset(data_root, ret_faces=ret_faces, ret_corr=False, ret_dist=True,
                                              ret_evecs=ret_evecs, num_evecs=num_evecs),
...
class SingleDT4DDataset(SingleShapeDataset):
    ...
    excluded, giving 177 train / 85 test shapes over 9 categories. Each category has one
    reference template; a frame's .vts has one line per reference point giving that frame's
    vertex. Maps are many-to-one and the template size differs per category.

    The raw dataset ships only off/ and corres/, so feats/dist/evecs default off; enable
    once the caches exist. Frame basenames repeat across categories, so item['name'] is
    '<category>_<frame>'.
    """
    def __init__(self,
                 data_root,
...
class PairDT4DDataset(_DT4DPairs, PairShapeDataset):
    ...
    def __init__(self,
        ...
        super().__init__(SingleDT4DDataset(data_root, phase, ret_faces, ret_corr,
...
class SparsePairDT4DDataset(_DT4DPairs, SparsePairShapeDataset):
    ...
    def __init__(self,
                 ...
                 fps_metric="geodesic"):
        self._init_dt4d(inter_class)
        super().__init__(SingleDT4DDataset(data_root, phase, ret_faces=True, ret_corr=True, ret_dist=True, ret_evecs=ret_evecs,
                                           num_evecs=num_evecs),
                         n_sparse=n_sparse, phase=phase, exclude_self=exclude_self,
                         fps_metric=fps_metric)
...
class PairScapeDataset(PairShapeDataset):
    def __init__(self,
                 ...
                 num_evecs=200,
                 exclude_self=False):
        dataset = SingleScapeDataset(data_root, phase, ret_faces, ret_corr, ret_dist, ret_evecs, num_evecs)
        super().__init__(dataset, exclude_self=exclude_self)
...
class SparsePairScapeDataset(SparsePairShapeDataset):
    ...
    def __init__(self,
                 ...
                 exclude_self=False,
                 fps_metric="geodesic"):
        dataset = SingleScapeDataset(data_root, phase, ret_faces=True, ret_corr=True, ret_dist=True, ret_evecs=ret_evecs,
                                     num_evecs=num_evecs)
        super().__init__(dataset, n_sparse=n_sparse, phase=phase, exclude_self=exclude_self,
                         fps_metric=fps_metric)
