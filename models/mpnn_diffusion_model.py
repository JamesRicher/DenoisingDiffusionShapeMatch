"""MPNN diffusion matcher: diffusion over sparse assignment matrices with an
MPNN denoiser.
"""
import os
...
class MPNNDiffusionModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        self.proj_iters = cfg.get('proj_iters', 5)      # Sinkhorn iterations
        ...
        self.reproject = cfg.get('reproject', False)    # reprojection an additional time per DDIM step
        ...
        # Sparse decode: hungarian = bijective assignment; argmax = per-row (non-bijective).
        # Independent of the projection mode.
        ...
        # which eval stats validation reports. Sparse (FPS-point geodesic error) is the
        # fast dev metric; dense whole-shape MGE is the reporting metric.
        ev = opt.get('eval', {})
        self.report_sparse = ev.get('sparse', True)
        self.report_dense = ev.get('dense', self.densifier is not None)
        # Dump every pair's predicted maps + per-pair MGE to <out_dir>/maps.npz. Off by
        # default: an eval-time artefact (evaluate.py --save_maps), not a per-epoch write.
        ...
        # optional Phase-3 diagnostics, run at validation when enabled
    ...
    def _read_in(self, u):
        """Projected read-in P_t (probabilities) the denoiser conditions on, via the Sinkhorn
        projection Pi_S. Shared by training and sampling."""
    ...
    def _final_snap(self, u):
        """Converged read of the final iterate (probabilities) handed to the decoder, via a
        longer-iteration Sinkhorn."""
    ...
    def _decode(self, P0):
        ...
        (Hungarian on the full matrix); argmax = per-row best match (non-bijective).
        Returns a CPU long tensor."""
    ...
    def _forward_ce(self, F_x, F_y, D_x, D_y, P0, u0, t):
        ...
        P_t = self._read_in(u_t)                                   # read-in (row-softmax or Pi_S)
    ...
    def feed_data(self, data):
        drop = self.is_train and self.feature_dropout > 0.0 \
                and torch.rand(1).item() < self.feature_dropout
    ...
    def sample(self, F_x, F_y, D_x, D_y, steps=None, return_trajectory=False, sample_eta=None):
        ...
        u = torch.randn(B, n, n, device=self.device)               # alpha_bar(1)=0 prior
    ...
    def _densify_context(self, data):
        ...
        tensors under the batch_size=1 single collate). Optional fields are left None when
        absent, so each densifier reads only what it needs. When the densifier wants model
        features, they come from a dense per-vertex forward of self.networks['extractor'],
        restored from the joint checkpoint -- the fully fine-tuned features, not the
        pretrained warm start."""
        x, y = data['first'], data['second']
        feat_x = feat_y = None
        if self.densifier is not None and getattr(self.densifier, 'wants_model_feats', False):
            ext = self.networks['extractor']
    ...
    def _pck_report(self, geo_errs, out_dir, tag=''):
        ...
        Writes pck<suffix>.png / pck<suffix>_data.npz to out_dir, in the shared
        baseline layout so curves from different methods overlay directly. Returns a dict
        of scalar metrics to fold into the validation result.
    ...
    def _save_maps(out_dir, names, mge, sparse, idx_x, idx_y, dense):
        ...
        Index keys rather than pair names keep the ragged dense members free of character
        and collision issues in the zip, and npz members decompress lazily, so rendering a
        few pairs out of a large file never touches the rest. int32 throughout, since int16
        would risk silent overflow above 32767 vertices for little size gain.
        """
    ...
    def validation(self, dataloader, out_dir=None):
        ...
        # Maps come from the dense pass only: it is the independent-FPS regime, and both the
        # sparse map and the dense map it densifies to are in hand there.
