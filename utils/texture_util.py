...
Synthesizes per-vertex UV coordinates on the source shape X, then carries them to
the target Y through the predicted map. Two variants are produced side by side:

  * hard:     uv_y = uv_x[p2p]                      (the map the metrics score)
  * smoothed: uv_y = Phi_y @ Cxy @ Phi_x^+ @ uv_x   (spectrally low-passed)

The smoothed transfer matches how published figures are rendered but hides
high-frequency error, so read the two together.

Rendering uses a small numpy software rasterizer (orthographic projection,
z-buffer, per-pixel barycentric UV interpolation) so texture.png is sampled at
full image resolution; per-vertex color sampling cannot resolve its grid labels.
"""
import os
...
def generate_tex_coords(verts, axes=None, preserve_aspect=False, scale=1.0):
    ...
    The default picks the two highest-variance axes. For an upright human that lands on
    the sagittal plane (height dominates, depth is usually the runner-up), so the texture
    reads as a side view; pass axes='xy' for a front-on projection.

    Args:
        verts (np.ndarray): vertex positions, [V, 3].
        axes (str, None): two of 'xyz' naming the (u, v) axes. None = variance order.
        preserve_aspect (bool): scale both axes by one factor so texture cells stay
            square. Default False stretches each axis to fill [0, 1].
        scale (float): multiply the normalized UVs, tiling the texture. Restores cell
            density when preserve_aspect leaves the short axis covering part of a tile.
    Returns:
...
def smoothed_uv_transfer(uv_x, p2p_yx, evecs_x, evecs_y, evecs_trans_x, evecs_trans_y):
    ...
    Cxy is estimated from the p2p map (Cxy = Phi_y^+ Phi_x[p2p]), then Pyx is applied
    without materializing the dense [Vy, Vx] matrix. Truncating to the eigenbasis
    low-passes the map, so pair this with hard_uv_transfer.
...
def checkerboard_image(res=512, n_cells=6):
    """Procedural fallback texture: a UV-driven color ramp modulated by a light/dark
    checker so texture direction stays readable. Returns RGB in [0, 1], [res, res, 3].
    """
...
def create_colormap(verts):
    """Per-vertex RGB from min-max-normalized vertex positions: a position-coded
    rainbow, so matching regions on the two shapes share a color after transfer.
    verts is [V, 3]; returns RGB in [0, 1], [V, 3].
    """
...
def rasterize_mesh_attribute(verts, faces, attr, attr_to_rgb, res=700,
    ...
    The attribute is barycentrically interpolated inside every triangle and mapped to RGB
    per pixel via attr_to_rgb, e.g. a texture lookup for UVs or the identity for vertex
    colors. Faces are lambert-shaded from their normals.
...
def rasterize_textured_mesh(verts, faces, uv, texture, **kwargs):
    """Render a mesh with per-pixel UV texture lookup, which is what makes the labelled
    texture grid legible. uv is [V, 2] in [0, 1]^2, texture is [H, W, 3] RGB in [0, 1];
...
def rasterize_colored_mesh(verts, faces, colors, **kwargs):
    """Render a mesh with smoothly interpolated per-vertex RGB colors, [V, 3] in [0, 1].
    See rasterize_mesh_attribute for the shared arguments.
    """
...
def render_texture_transfer_figure(verts_x, faces_x, verts_y, faces_y, p2p_yx,
    ...
    The smoothed panel is drawn only when all four spectral arrays are given, otherwise
    the figure has two panels.

    Args:
        verts_x, faces_x, verts_y, faces_y (np.ndarray): the two meshes.
        p2p_yx (np.ndarray): predicted point map Y -> X, [Vy].
        evecs_* / evecs_trans_* (np.ndarray, optional): spectral quantities for the
            smoothed transfer (see smoothed_uv_transfer).
        flip_up (bool): dataset up-axis flag (True for SMAL).
        res (int): per-panel raster resolution.
        title (str, optional): figure suptitle.
        texture_file (str): texture sampled at the UVs; falls back to a checkerboard.
        out_file (str, optional): save path; the figure is closed after saving and None
            returned. When omitted the open figure is returned.
    """
