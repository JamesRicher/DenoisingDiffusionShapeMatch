...
def consistent_bijective_fps(verts_x: np.ndarray, corr_x: np.ndarray, corr_y: np.ndarray,
    ...
    The template point -> vertex map is many-to-one per shape, so corr_x[K] and corr_y[K]
    can collide. FPS on X's covered points gives a well-spread ordering and
    _greedy_bijective keeps only positions distinct on both shapes, making the GT an exact
    permutation. FPS depth grows until n points are collected.
