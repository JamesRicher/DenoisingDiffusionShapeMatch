...
class MetricLogger:
    ...
    The CSV (results/metrics.csv) is the source of truth and can be plotted at any
    time; the TensorBoard event files (under experiments/<name>/tb/) give
    live/interactive monitoring. TensorBoard is optional: if the package is not
    installed, CSV logging continues and a single warning is emitted.

    Tags are free-form, grouped by prefix: use Loss/* for loss terms and Val/* for
    validation metrics (e.g. Val/avg_error).
    """
